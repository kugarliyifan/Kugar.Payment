...
namespace Kugar.Payment.Wechatpay.Services
{
    ...
    /// 通知处理
    /// </summary>
    public class NotifyHandlerService: AliServiceBase
    {
        public NotifyHandlerService(Alipay.Alipay pay, AlipayConfig config) : base(pay, config)
        {
        }
        ...
        public async Task<ResultReturn<NotifyPaymentResult>> DecodePaymentNotifyData(string xml)
        {
            ...
            if (!await Parent..QueryByTransactionId(data.TransactionId))
            {
